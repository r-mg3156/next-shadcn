...
const getFileContent = async (file: z.infer<typeof registryItemFileSchema>) => {
...
const getFileTarget = (file: z.infer<typeof registryItemFileSchema>) => {
...
const fixFilePaths = (files: z.infer<typeof registryItemSchema>["files"]) => {
  if (!files) {
    return [];
  }
